...
read -r -d '' HELP <<EOF
my_product test runner. Execute unit test suite and some other details.
...
shift $((OPTIND - 1))
if [ -n "${1}" ]; then
    TEST_FILE="Web/typo3conf/ext/my_product/${1}"
else
    case ${TEST_SUITE} in
        acceptance)
            TEST_FILE="Web/typo3conf/ext/my_product/Tests/Acceptance"
            ;;
        functional)
            TEST_FILE="Web/typo3conf/ext/my_product/Tests/Functional"
            ;;
        unit)
            TEST_FILE="Web/typo3conf/ext/my_product/Tests/Unit"
            ;;
    esac
fi
